...
import { BASE_CHECK_RATE, MAX_CHECK_RATE, PRIORITY_ALWAYS_CHECK } from '../config/constants.js';
...
export const getCheckRateForPriority = (priority) => {
  ...
  const clampedPriority = Math.max(0, Math.min(100, p));
  ...
  const k = 4; // Steepness factor (reduced to compensate for base rate)
  const normalizedPriority = clampedPriority / 100;
  const additionalTime = (MAX_CHECK_RATE - BASE_CHECK_RATE) * Math.pow(2, -k * normalizedPriority);
